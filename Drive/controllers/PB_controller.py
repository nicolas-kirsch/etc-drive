...
class PerfBoostController(nn.Module):
    ...
    def reset(self,batch_size=None):
        ...
        self.last_disturbance_t = torch.full((batch_size,1,1),-1000).to(device)   # time step of the last disturbance occurrence
        
        self.v_beta_prev = torch.full((batch_size,1,1),0).to(device)
        self.v_alpha_prev = torch.full((batch_size,1,1),3150).to(device)
        self.sign_change_happened = torch.zeros((batch_size,1,1),device=device).bool()
        #self.emme.reset()
