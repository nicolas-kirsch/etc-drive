...
class Converter(torch.nn.Module):
    def __init__(self,x0,wref,vref,Qref,h, M, D,C,G,lg,z,l,base_values,u_init = None):
        ...
        self.vg[:,:,0] = 3000/base_values["V_base"]
        self.vg[:,:,1] = 0
    ...
    def compute_i_ref(self,P_ref: torch.Tensor,u_PB:torch.Tensor,v_dc:torch.Tensor,vg:torch.Tensor):
            """
            output of the base controller
    ...
    def noiseless_forward(self, x: torch.Tensor,u_PB: torch.Tensor,d_mech: torch.Tensor,vg:torch.Tensor, no_PB=False):
        ...
        i_ref = self.compute_i_ref(P_ref,d_Q,v_dc,vg)
        ...
        ig_ = F.linear(ig,torch.eye(2)-self.h*self.Lg_inv@self.Z) - self.h*F.linear(mg*v_dc,self.Lg_inv) + self.h*F.linear(vg,self.Lg_inv)
    ...
    def rollout(self,controller,data: torch.Tensor, no_PB = False):
        ...
        for t in range(1, data.shape[1]):
            ...
            u_PB = torch.cat(
                (u_PB, controller.forward(xs[:, t:t+1, :6],d_mech[:, t:t+1, :],data[:,t:t+1,2:4])),
                1
            )
