...
save_path = os.path.join(BASE_DIR, 'experiments', 'Converter_wFD', 'saved_results',n_phase)
...
alpha_i_max = args.alpha_i_max
msg = 'R: %.2f --- Q_Q: %.2f --- Q_vdc: %.2f --- alpha_v_max: %.2f --- alpha_i_max: %.2f' % (R, Q_Q, Q_vdc, alpha_v_max,alpha_i_max)
...
loss_fn = ConverterLoss(
    R=R,Q_Q=Q_Q,Q_vdc=Q_vdc, x_min=x_min,yref=yref,imax = imax, alpha_v_max=alpha_v_max, alpha_i_max=alpha_i_max  
)
...
x_log_base,us,u_PB,d_mech = sys.rollout(ctl,train_data, no_PB=True)
...
with torch.no_grad():
    x_log_test, u_log_test, pb,_ = sys.rollout(
        controller=ctl, data=train_data, no_PB=False
    )
...
axs[0, 0].axhline(y=4900, linestyle="--", color="teal", label=r"Acceptable range")
axs[0, 0].axhline(y=5100, linestyle="--", color="teal")
