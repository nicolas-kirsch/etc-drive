...
from plants import ConverterDatasetFD, ConverterFD
...
save_path = os.path.join(BASE_DIR, 'experiments', 'Converter_wFD', 'saved_results')
save_folder = os.path.join(save_path, '1_phases_lost/perf_boost_01_17_18_40_19')
...
for t in range(test_data.shape[1]):
    ...
    if fault_phase == 1:
        v_alpha = (2/3) * (va_loss - 0.5*vb - 0.5*vc)
        v_beta  = (2/3) * ((np.sqrt(3)/2)*vb - (np.sqrt(3)/2)*vc)
    
    elif fault_phase == 2:
        v_alpha = (2/3) * (va - 0.5*vb_loss - 0.5*vc)
        v_beta  = (2/3) * ((np.sqrt(3)/2)*vb_loss - (np.sqrt(3)/2)*vc)
...
sys  = ConverterFD(x0,wref,vref,qref,h,m,d,c,g,lg,z,l,base_values,Ared,Bred,Cred,Ered)
...
plt.plot(np.array(range(vdc_log_base.shape[0]))*h, vdc_log_base,color = "#FF7F0E", label = "Base")
plt.plot(np.array(range(vdc_log_base.shape[0]))*h, vdc_log_test, label = "PB")
plt.axhline(y=5000, linestyle="--", color="red", label=r"$v_{dc}^*$")
plt.axhline(y=4870, linestyle="--", color="teal", label=r"Acceptable range")
plt.axhline(y=5130, linestyle="--", color="teal")
plt.title("Full loss of phase C: DC-link voltage response", fontsize=18)
plt.xlabel("Time (s)")
...
"""
# Plot 3: U profile over the horizon
axs[2, 1].plot(np.array(range(test_data.shape[1]))*h, us[0,:,:])
