...
class ConverterFD(torch.nn.Module):
    ...
    def rollout(self,controller,data: torch.Tensor, no_PB = False):
        ...
        d = data[:,:,self.state_dim:self.state_dim*2]
        x = w[:,0:1,:].to(device)
        ...
        u_PB = controller.forward(xs[:, 0:1, :],d[:, 0:1, :],vg,context[:,0:1,:],init = True)
        ...
        for t in range(1, data.shape[1]):
            ...
            u_PB = torch.cat(
                (u_PB, controller.forward(xs[:, t:t+1, :],d[:, t:t+1, :],data[:,t:t+1,self.state_dim+2:self.state_dim+4], context[:,t:t+1,:], no_PB=no_PB)),
                1
            )
            """u_PB = torch.cat(
