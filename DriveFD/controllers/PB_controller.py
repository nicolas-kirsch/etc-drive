...
class PerfBoostController(nn.Module):
    ...
    def __init__(
        ...
        self.dim_out = 2
        dim_in_ren = self.dim_in+3+1+2 +4 # +1 for vdc_ref
        self.Vbase = 5000
        self.wbase = 126.66
        self.ibase = 2222.2
        ...
        self.MLP = MLP(dim_in = 20, dim_out = self.dim_out)
    ...
    def forward(self, input_t: torch.Tensor,d: torch.Tensor,vg:torch.Tensor,context:torch.Tensor,init = False, no_PB = False):
        """
        ...
        time_encoding = context[:,:,-4:-2]*is_disturbance
        label_scale = context[:,:,-2:]
        #Get the current error
        error_vdc = (input_t[:,:,0:1] - self.vdc_ref)  # shape = (self.batch_size, 1, 1)
        
        ren_input = torch.cat((w_,dist_presence,v_norm_dist,vg_pu,time_encoding,label_scale), dim=2)


        mlp_input = torch.cat((w_[:,:,0:4],d,input_t,context), dim=2)
