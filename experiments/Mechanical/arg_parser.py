...
def argument_parser():
    ...
    # dataset
    parser.add_argument('--horizon', type=int, default=int(3e+4), help='Time horizon for the computation. Default is 24.')
    parser.add_argument('--state-dim', type=int, default=1, help='Number of states of the LTI Plant. Default is 1.')
    parser.add_argument('--num-rollouts', type=int, default=60, help='Number of rollouts in the training data. Default is 30.')

    # optimizer
    parser.add_argument('--batch-size', type=int, default=60, help='Number of forward trajectories of the closed-loop system at each step. Default is 5.')
    parser.add_argument('--epochs', type=int, default=20, help='Total number of epochs for training. Default is 5000 if collision avoidance, else 100.')
    ...
    if args.log_epoch == -1 or args.log_epoch is None:
        args.log_epoch = math.ceil(float(args.epochs)/4)
